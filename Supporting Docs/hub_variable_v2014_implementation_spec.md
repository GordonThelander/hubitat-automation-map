...
**Depends on:** `hub_variable_first_class_spec.md`
...
changes.
**Evidence basis:** live-tested `getAllGlobalVars()`/
`getGlobalVar()` on the dev hub, 2026-08-26; a code-level review of `apps/automation_map.groovy`
`extractHubVariableWrites()` finding independently confirmed against source the same day).

## Revision history

- **2026-08-26 (rev 2):** Review of the original draft approved
...
- **2026-08-26 (rev 1):** Initial draft, sent for review.
...
v2.0.14 adds one new discovery call - `getAllGlobalVars()`, confirmed live and general-purpose - and reconciles it with the existing decoded references, per section 6.3 of the parent spec.
...
2. **Hub Variable node model** (parent spec 7.1), populated from the confirmed live shape:
   ...
   - **Type normalization, defined explicitly (review point 6):** map known platform type spellings
     ...
     fixture, not assumed.
   - **Identity rule, stated explicitly (review point 6):** no separate stable ID was found live
...
3. **Reconciliation** (parent spec 6.3), restated precisely per review point 5:
...
4. **Scan-state metadata** (parent spec 6.1): `hubVariableInventory{status,error,count,source}` and
   ...
   - **Transactional publication (review point 7):** the `getAllGlobalVars()` call is synchronous,
...
5. **Export schema 4** (parent spec 11.1-11.4): root additions, the Hub Variable record shape, and
   ...
   - **No dual-export mode (review point 4):** this is a clean advance to schema 4, not a
...
6. **`writeSource` - corrected (review point 1, verified against source 2026-08-26).** The original
...
7. **Connector / `synchronizedWith` edges - moved in scope (review point 2).** Resolves the original
...
8. **`insights.hubVariables` findings - moved in scope (review point 3).** Ship the complete
...
(section 2, items 7-8) per review. What remains out of scope:
...
## 4. Named code changes (for review, not yet written)
...
- **Connector-present case - confirmed live 2026-08-26, with a real platform finding.**
  ...
  exactly as expected once deployed. Known, accepted trade-off, confirmed with review: a
  ...
  field (review required cleanup item 1) - the orphan/stale-ID gap is now documented only
...
1. the review reviews this document and either agrees, proposes amendments, or flags disagreement.
   (Done - corrections incorporated above.)
