...
- **`minimumHEVersion` (2.3.0) is a floor for the core map only.** It has not been separately verified for Visual Rule Builder 2.0 decoding, which is itself still beta on Hubitat's side, or for `/hub2/appsList` discovery. Both degrade gracefully if unavailable - undecoded engines are still counted and reported, and a missing app list falls back to device-led discovery - so this is a documentation gap, not a known crash risk.
