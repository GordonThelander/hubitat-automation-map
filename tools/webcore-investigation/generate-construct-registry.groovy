...
// Statement ceilings come from committed L3 evidence metadata, never from a test run.
File l3Root = new File('.').canonicalFile
Class l3Promotion = new GroovyClassLoader(this.class.classLoader)
    .parseClass(new File(l3Root, 'tools/webcore-investigation/L3Promotion.groovy'))
Map l3 = l3Promotion.derive(
    new GroovyShell().evaluate(new File(l3Root, 'tools/webcore-investigation/evidence/statement-l3.groovy').getText('UTF-8')) as Map,
    new groovy.json.JsonSlurper().parse(new File(l3Root, 'tests/fixtures/webcore-l3/manifest.json')) as Map,
    l3Root) as Map
if (l3.problems) {
    System.setOut(realOut)
    System.err.println "GATE: L3 promotion claims do not hold: ${(l3.problems as List).take(5)}"
    System.exit 1
}

// Operand ceilings reuse the same gate through a shim: the class only ever reads
// evidence.statements as its family list and evidence.captureLineage/namedTests for
// lineage and test resolution, so operand-l3.groovy's own operands/captureLineage/
// namedTests slot straight in. countsField='operands' points it at each fixture's
// own operand occurrence counts instead of its statement counts.
Map operandEvidence = new GroovyShell().evaluate(new File(l3Root, 'tools/webcore-investigation/evidence/operand-l3.groovy').getText('UTF-8')) as Map
Map operandL3 = l3Promotion.derive(
    [captureLineage: operandEvidence.captureLineage, namedTests: operandEvidence.namedTests, statements: operandEvidence.operands],
    new groovy.json.JsonSlurper().parse(new File(l3Root, 'tests/fixtures/webcore-l3/manifest.json')) as Map,
    l3Root, 'operands') as Map
if (operandL3.problems) {
    System.setOut(realOut)
    System.err.println "GATE: operand L3 promotion claims do not hold: ${(operandL3.problems as List).take(5)}"
    System.exit 1
}
...
statements.each { String st ->
    sb << entry("${NS}statement.${st}", [kind: 'statement', name: st, declared: true, implemented: true,
        canonicalTarget: null, availability: 'current', level: ((l3.levels as Map)["${NS}statement.${st}".toString()] ?: 'L2'),
...
canonical.sort().each { String id, Map c ->
    ...
    sb << entry(id, [kind: 'operand', name: c.name,
        ...
        level: ((operandL3.levels as Map)[id] ?: 'L2'),
