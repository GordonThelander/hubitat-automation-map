...
[
  ...
  openQuestions: [
    ...
    'the empty operand (wc.operand.empty) is source-proven but appears not to be producible from the current hosted editor, so it has no fixture evidence and stays L2. Resolved 2026-09-12 by a direct capture (zz-L3-18: a Make a web request task with its three optional parameters left untouched, paused throughout, first save plus unchanged round trip). Two states the registry models separately had been conflated in earlier notes here. wc.operand.empty is an operand whose t is the empty string: a real dispatch member at executor.evaluate-operand (case sBLK, commented "optional, nothing selected", returning rtnMap(ovt,null)), where sBLK is declared as the empty string and the only key consumed is vt, read via ovt=sMvt(operand). wc.task-parameter.unselected is a saved task parameter carrying no t at all, which is the state the ty==sNL branch in executor.clean-code addresses, where sNL is declared as (String)null. The editor holds an empty-string t in memory for an untouched optional parameter (editor.validate-operand assigns the empty string only when the operand is optional, and a command parameter is optional iff it is non-bool and carries a description d) and renders that state as "(no value set)". The empty string does not survive to storage: both captures saved the three optional parameters as {d: [], f: l, g: avg, vt: ...} with no t key at all, scoring wc.task-parameter.unselected three times and wc.operand.empty zero times. So the empty operand is a shape the executor will accept but the current editor never writes; evidence for it would have to come from an older saved piston or a non-editor write path. A second confirmation from the same capture: d and g both survive the canonical reload on those typeless parameters, so the item.remove(sD)/item.remove(sG) strip really is inMem-guarded and does not reach the stored copy',
