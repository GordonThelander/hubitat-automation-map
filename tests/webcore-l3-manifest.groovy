...
List premature = st.findAll { id, e -> ((e as Map).fixtures as List) || ((e as Map).tests as List) }.keySet().collect { "${it}" }
check(premature.isEmpty(), "no statement names fixtures or tests before A2 ${premature}")
List raised = registryStatements.findAll { (constructs[it] as Map).level != 'L2' }.toList()
check(raised.isEmpty(), "every statement is still L2 in the registry ${raised}")
